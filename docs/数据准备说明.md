...
对应 [#1 标注与样本整理](https://github.com/hycx233/deep-learning/issues/1)，为 [#2 正式预处理](https://github.com/hycx233/deep-learning/issues/2)提供真实输入。已在 Python 3.12.14 / Linux 上用课程原始数据运行；本步骤不使用 GPU，不训练模型。#2 的后续实现与正式分组见[共享预处理说明](共享预处理说明.md)，本页保留 #1 原始预览的用途说明。
...
- 共享归一化、环状边界处理、128×128 模型输入和防止空间重叠泄漏的数据分组由后续 #2 入口 `prepare_data.py` 提供。分类入口 PR #13 应接入其中的 `structure_id` 和正式 `split`，不能只凭不同结构 ID 就认定相互重叠的窗口独立。
